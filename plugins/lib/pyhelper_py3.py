...
class external(object):
    ...
    def run(cmd, shell=True):
        # type: (List[str], bool) -> Tuple[bytes, bytes, int]
        tmp = subprocess.Popen([str(x) for x in cmd], shell=shell, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class livestatus(object):
    @staticmethod
    def query(msg, encoding='utf-8'):
        # type: (str, str) -> Union[str, int, Tuple, List, Dict, bool, None]
        import ast
        ...
        while True:
            ...
            if not data:
                if not total_data:
                    total_data = ['[[]]']
                break
            total_data.append(data.decode(encoding))

        s.close()
        return ast.literal_eval(''.join(total_data))

    @staticmethod
    def query_tcp(msg, ip, port=6557, encoding='utf-8'):
        # type: (str, str, int, str) -> Union[str, int, Tuple, List, Dict, bool, None]
        import ast
